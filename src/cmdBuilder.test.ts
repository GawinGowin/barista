...
describe('test cmdBuilder', () => {
    it('verify CMD for cmdDependsOn', () => {
        expect(cmdBuilder.cmdDependsOn({ thisFile: 'file1', thatFile: 'file2' }, 0)).toBe('MERGE (a:File { name: "file1" }) ON CREATE SET a.name = "file1" MERGE (b:File { name: "file2" }) ON CREATE SET b.name = "file2" MERGE (a)-[:DEPENDS_ON]->(b);');
    });
